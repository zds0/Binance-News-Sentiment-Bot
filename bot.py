...
class BinanceTradeBot(BnClient):
    ...
    def __init__(self, ak: str, sk: str, proxies: dict, test: bool):
        if proxies == {}:
            ...
            BnClient.__init__(
                ...
                api_key=ak,
                api_secret=sk,
                requests_params={"proxies": proxies, "verify": False, "timeout": 10},
            )
        if test:
    ...
    def is_ready(self):
        ...
        try:
            ...
            if ready:
                if self.test:
                    logger.info(
                        "BinanceTradeBot has got ready now, let's hack with the test net."
                    )
                else:
                    logger.info(
                        "BinanceTradeBot has got ready now, let's hack with the main net."
                    )
            return ready
            ...
            if info != None:
                ...
                for flt in filters:
                    if flt["filterType"] == "LOT_SIZE":
                        ...
                        logger.info(
                            "LOT SIZE of {} is {}".format(
                                coin + self.PAIRING, step_size
                            )
                        )
                        break
            else:
                logger.warning(
                    "Failed to fetch information about {}".format(coin + self.PAIRING)
                )
            ...
            for url in self.feed_list:
                try:
                    ...
                    if r.status_code != 200:
                        logger.error(
                            "Failed to parse rss feed {}, status code: {}",
                            url,
                            r.status_code,
                        )
                    else:
    ...
    def __compile_sentiment(self):
        ...
        for coin in sentiment.keys():
            ...
            for item in sentiment[coin]:
                compiled_sentiment[coin].append(
                    sentiment[coin][sentiment[coin].index(item)]["compound"]
                )
    ...
    def buy(self, compiled_sentiment, analysed_headlines):
        ...
        for coin in compiled_sentiment.keys():
            if (
                compiled_sentiment[coin] > self.SENTIMENT_THRESHOLD
                and analysed_headlines[coin] >= self.MINIMUM_ARTICLES
            ):
                log.info(
                    "Preparing to buy {} {} with {} at {}".format(
                        volume[coin + self.PAIRING],
                        coin,
                        self.PAIRING,
                        self.current_price[coin + self.PAIRING],
                    )
                )
                try:
                    ...
                    try:
                        ...
                        logger.info(
                            "Order {} has been placed on {} with {} coins at {} and bought at {}".format(
                                order[0]["orderId"],
                                coin,
                                order[0]["origQty"],
                                utc_time,
                ...
                logger.warning(
                    "Sentiment is not positive enough for {}, or not enough headlines analysed".format(
                        coin
                    )
                )
    ...
    def sell(self, compiled_sentiment, analysed_headlines):
        for coin in compiled_sentiment.keys():
            if (
                compiled_sentiment[coin] < self.NEGATIVE_SENTIMENT_THRESHOLD
                and analysed_headlines[coin] >= self.MINIMUM_ARTICLES
                and self.asset_balance[coin] > 0
            ):
                log.info(
                    "Preparing to sell {} {} at {}".format(
                        self.asset_balance[coin],
                        coin,
                        self.current_price[coin + self.PAIRING],
                    )
                )
                try:
                    ...
                    try:
                        ...
                        logger.info(
                            "Order {} has been placed on {} with {} coins at and sold for {}".format(
                                order[0]["orderId"],
                                coin,
                                order[0]["origQty"],
                                utc_time,
                ...
                logger.warning(
                    "Sentiment is not negative enough for {}, or not enough headlines analysed or not enough {} to sell".format(
                        coin, coin
                    )
                )
